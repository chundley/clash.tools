...
function ($rootScope, $scope, $routeParams, $location, authService, sessionService, errorService, marketoService, workflowService, messagelogService, RESPONSE_TYPES, SIFTROCK_FIELDS) {
    ...
    $scope.partner = 'marketo';

    $scope.actionGroups = {};
    $scope.actionGroups['marketo'] = {
        groupName: 'Marketo',
        actions: [
    ...
    sessionService.getCurrentAccount(authService.user.id, function (err, account) {
        if (err) {
            err.stack_trace.unshift( { file: 'marketo-controller.js', func: 'init', message: 'Error getting current account' } );
            ...
            if ($routeParams.id === 'new') {
                ...
                workflowService.getById(account._id, $routeParams.id, function (err, workflow) {
                    if (err) {
                        err.stack_trace.unshift( { file: 'marketo-controller.js', func: 'init', message: 'Error getting existing workflow' } );
                        errorService.save(err, function() {});
                    }
                    else {
    ...
    */
    $scope.changeAction = function(action) {
        ...
        if ($scope.workflowState.addToList && !$scope.lists) {
            marketoService.getLists($scope.account._id, function (err, lists) {
                $scope.lists = lists;
            });
        }
    ...
    $scope.saveWorkflow = function() {
        ...
        workflowService.save($scope.workflow, function (err, result) {
            if (err) {
                err.stack_trace.unshift( { file: 'marketo-controller.js', func: 'saveWorkflow', message: 'Error saving workflow' } );
                errorService.save(err, function() {});
            }
            else {
                messagelogService.save(
                    ...
                    function (err, res) {
                        if (err) {
                            err.stack_trace.unshift( { file: 'marketo-controller.js', func: '$scope.saveWorkflow', message: 'Error saving log message' } );
                            errorService.save(err, function() {});
                        }
                    }
                );
    ...
    */
    ...
    /*
    *   Some of the fields in Marketo are only updated by the Siftrock jobs service - don't make them
    *   available to the UI
    */
    function filterFields(marketoFields) {
        ...
        angular.forEach(marketoFields, function (marketoField) {
            var found = false;
            angular.forEach(reqFields, function (reqField) {
                if (reqField === marketoField.name) {
                    found = true;
                }
            });

            if (found) {
                marketoField.visible = false;
            }
            else {
                marketoField.visible = true;
            }
        });
